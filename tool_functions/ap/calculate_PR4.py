...
files = np.load('test_ap_0.690.npy', allow_pickle=True)
data4ap = []

for gts, dts in files:
    gt_boxes = np.array(gts)
    dt_boxes = np.array(dts[0])
    scores = np.array(dts[1])
    # print(gt_boxes, dt_boxes, scores)

    data4ap.append([gt_boxes, dt_boxes, scores])
...
class CocoEvaluator(object):
    ...
    def summarize(self):
        for iou_type, coco_eval in self.coco_eval.items():
            ...
            Metric = '{} @ IoU={:<9} | maxDets={:>3d} = {:0.3f}\n' \
                     '{} @ IoU={:<9} | maxDets={:>3d} = {:0.3f}\n' \
                     '{} @ IoU={:<9} | maxDets={:>3d} = {:0.3f}\n' \
                     '{} @ IoU={:<9} | maxDets={:>3d} = {:0.3f}\n' \
                     '{} @ IoU={:<9} | maxDets={:>3d} = {:0.3f}\n' \
                     '{} @ IoU={:<9} | maxDets={:>3d} = {:0.3f}\n'.format(
                'AP', '0.50:0.95', 100, coco_eval.stats[0],
                'AP', '0.50',      100, coco_eval.stats[1],
                'AP', '0.75',      100, coco_eval.stats[2],
                'AR', '0.50:0.95',   1, coco_eval.stats[6],
                'AR', '0.50:0.95',  10, coco_eval.stats[7],
                'AR', '0.50:0.95', 100, coco_eval.stats[8],
            )
            logger.info(Metric)
            AP_iou05 = coco_eval.stats[1]
        return AP_iou05
...
def calculateAP(data_):
    ...
    plt.ylim(0, 1.01)
    plt.grid(True)
    plt.plot(x, pr_array1, 'b-', label='AP 0.5')
